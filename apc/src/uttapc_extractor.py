...
class APCStatPoolingLayer(torch.nn.Module):
    ...
    def forward(self, feat, length, device):
        ...
        if self.merge_mode == "concat":
            # Concatenation + Linear layer [Need to modify so that the approach don't OOM]
...
"""
LSTM + Utterance-level Modelling Model(Extractor):
Params:  input_size  -> dimension of input melspectogram
         ...
         sq_ratio        -> reduction ratio for squeeze-and-excitation
         utterance_level_dropout ->  dropout on std before pooling
         extract_layer   -> layer that we want to extact
""" 
class UtteranceLevelAPC(torch.nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout=0.0, 
                 config_residual=True, use_var=True, merge_mode="squeeze", sq_ratio = 2, utterance_level_dropout=None, extract_layer=3):
        super().__init__()
    ...
    def forward(self, feat, length, device):
        ...
        for i, layer in enumerate(self.lstms):
            ...
            if i+1 == self.extract_layer:
                #TODO[Write mode for extracting utterance level embedding]
                mean_preframes = torch.sum(pre_frm_embeddings,axis=1)*1/length.unsqueeze(1)
                utt_embeddings = utt_embeddings.squeeze(1)
                mean_postframes = torch.sum(post_frm_embeddings,axis=1)*1/length.unsqueeze(1)
                return  mean_preframes, utt_embeddings, mean_postframes