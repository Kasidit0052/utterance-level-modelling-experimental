...
for ((i = $epoch_start; i <= $epoch_end; i++)); do
    if [[ $i == 1 ]]; then 
        ...
        if test -f "${exp_folder}/param-0"; then
            ...
            echo -e "Epoch 0: Command[srun -J $job_name bash train-old-epoch.sh $exp_folder 0]"
            srun -J $job_name bash train-old-epoch.sh $exp_folder 0
            wait
        fi
        echo -e "\n"
    ...
    if test -f "${exp_folder}/param-${i}"; then
        ...
        echo -e "Epoch ${i}: Command[srun -J $job_name bash train-old-epoch.sh $exp_folder $i]"
        srun -J $job_name bash train-old-epoch.sh $exp_folder $i
        wait
    fi
