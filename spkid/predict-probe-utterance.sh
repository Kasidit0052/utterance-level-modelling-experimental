...
for ((i = $epoch_start; i <= $epoch_end; i++)); do
    ...
    if test -f "${exp_folder}/dev-${i}-eval.log"; then
        ...
        echo -e "Epoch ${i}: Command[srun -J $job_name bash predict-probe-utterance-epoch.sh $exp_folder $i]"
        srun -J $job_name bash predict-probe-utterance-epoch.sh $exp_folder $i
        wait
    fi
