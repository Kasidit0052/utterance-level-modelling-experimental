...
"""
LSTM + Utterance-level Modelling Downstream utterance Classification:
Params:  input_size  -> dimension of input melspectogram
         ...
         merge_mode      -> augment mode ["residual", "concat", "squeeze"]
         sq_ratio        -> reduction ratio for squeeze-and-excitation
         extract_mode    -> extract location ["postframe","utterance-vector"]
         utterance_level_dropout ->  dropout on std before pooling
         extract_layer   -> Depth where we want to extract feature
""" 
class UttAPCSpkLstm(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers, dropout=0.0, 
                 ...
                 extract_mode='post-frame',utterance_level_dropout=None,extract_layer=3):
        super().__init__()
    ...
    def forward(self, feat, length, device):
        ...
        for i, layer in enumerate(self.lstms):
            ...
            if i+1 == self.extract_layer:
                ...
                if self.extract_mode == 'utterance-vector':
                    utt_embeddings = utt_embeddings.squeeze(1)
                    return utt_embeddings, self.pred(utt_embeddings)
                if self.extract_mode == 'post-frame':
                    utt_embeddings = torch.sum(post_frm_embeddings,axis=1)*1/length.unsqueeze(1)
                    return utt_embeddings, self.pred(utt_embeddings)
                # else-case
                return utt_embeddings, self.pred(utt_embeddings)
